...
composits :: [Integer]
composits = filter (not . prime) [2..]

-------------------------- Excersise 4 --------------------------------
-- Time spent:  2,5 hours

testPrimeTestF k (x:xs) = do value <- primeTestsF k x
                             if value
                                then do
                                    return (False, x)
                             else testPrimeTestF k xs

testPrimeOne = testPrimeTestF 1 composits
testPrimeTwo = testPrimeTestF 2 composits
testPrimeThree = testPrimeTestF 3 composits
testPrimeFour = testPrimeTestF 4 composits
testPrimeFive = testPrimeTestF 5 composits
testPrimeSix = testPrimeTestF 6 composits
testPrimeSeven = testPrimeTestF 7 composits

-- testPrime1000Times = map (testPrimeOne) [1..1000]
-- Expanding K will make the test run longer and the return value gets bigger.
-- The least number to fool the test I have found is the number 9.
...
testPrimeCarMichael = testPrimeTestF 1 carmichael


-- The fermits prime check checks if for a certain property of a prime number. Making it a test for psuedo primes. The carmicheal sequence in number theory
-- Generates a list of all psuedo primes that have the property of the function of the fermits prime check.

------------------------ Excersise 6 --------------------------------


testMillerRabin k (x:xs) = do value <- primeMR k x
                              if value
                                 then do
                                     return (False, x)
                              else testMillerRabin k xs

testPrimeMillerRabin = testMillerRabin 1 carmichael
